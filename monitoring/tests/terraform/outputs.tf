...
output "win_gen2_custom_id" {
  value = {
    for winc2 in azurerm_windows_virtual_machine.custom_win_vm_gen2:
        winc2.id => winc2.id
    }
}
...
output "linux_gen2_custom_ubuntu_id" {
  value = {
    for lnxuc2 in azurerm_linux_virtual_machine.custom_ubuntu_vm_gen2:
        lnxuc2.id => lnxuc2.id
    }
}
