...
resource "azurerm_linux_virtual_machine" "custom_ubuntu_vm" {
    for_each = {
    for ubuntu_vm_name, ubuntu_vm_properties in var.custom_image_ubuntu_vm_list : ubuntu_vm_name => ubuntu_vm_properties
    if ubuntu_vm_properties.vm_gen == "1"
  }
...
resource "azurerm_windows_virtual_machine" "custom_win_vm" {
  for_each = {
    for windows_vm_name, windows_vm_properties in var.custom_image_windows_vm_list : windows_vm_name => windows_vm_properties
    if windows_vm_properties.vm_gen == "1"
  }
...
## Custom Image Windows VM Gen2 ##
data "azurerm_shared_image_version" "img" {
  for_each = var.shared_image_definition
...
resource "azurerm_windows_virtual_machine" "custom_win_vm_gen2" {
  for_each = {
    for windows_vm_name, windows_vm_properties in var.custom_image_windows_vm_list : windows_vm_name => windows_vm_properties
    if windows_vm_properties.vm_gen == "2"
  }
  ...
  admin_password        = var.admin_password
  provision_vm_agent    = true
  source_image_id       = data.azurerm_shared_image_version.img[each.value.os_name].id
...
## Custom Image Linux Ubuntu VM ##
resource "azurerm_linux_virtual_machine" "custom_ubuntu_vm_gen2" {
  for_each = {
    for ubuntu_vm_name, ubuntu_vm_properties in var.custom_image_ubuntu_vm_list : ubuntu_vm_name => ubuntu_vm_properties
    if ubuntu_vm_properties.vm_gen == "2"
  }
...
resource "azurerm_dev_test_global_vm_shutdown_schedule" "win_linux_vms_auto_shutdown" {
  ...
  depends_on = [azurerm_windows_virtual_machine.win_vm, azurerm_windows_virtual_machine.custom_win_vm, azurerm_windows_virtual_machine.custom_win_vm_gen2, azurerm_linux_virtual_machine.linux_vm, azurerm_linux_virtual_machine.custom_ubuntu_vm, azurerm_linux_virtual_machine.custom_ubuntu_vm_gen2, azurerm_linux_virtual_machine.custom_rhel_vm]
